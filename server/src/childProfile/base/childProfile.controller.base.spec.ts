...
const CREATE_INPUT = {
  ...
  id: "exampleId",
  nameOfKeyWorker: "exampleNameOfKeyWorker",
  startDate: new Date(),
  updatedAt: new Date(),
};
const CREATE_RESULT = {
...
describe("ChildProfile", () => {
  ...
  test("GET /childProfiles/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/childProfiles"}/${nonExistingId}`)
  ...
  test("GET /childProfiles/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/childProfiles"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
